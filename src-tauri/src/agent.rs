...
impl ResearchAgent {
    ...
    pub async fn quick_search(&self, query: &str) -> Result<String> {
        let results = self.quick_search_results(query).await?;
        if results.is_empty() {
    ...
    /// Return raw search results (no formatting). Used when the command returns structured JSON.
    pub async fn quick_search_results(&self, query: &str) -> Result<Vec<SearchResult>> {
